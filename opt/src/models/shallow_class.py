...
class ShallowClassifier:
    ...
    def run_classifier(self, X):
        ...
        for it, (lr_idx, ts_idx) in enumerate(loo):
            ...
            X_train = learning_set.drop(self.info.project.target.aim, axis=1).values
            y_train = learning_set[self.info.project.target.aim].values
            ...
            scores[f'{it}'] = evaluate(y_trues, y_preds, y_pred_probas, self.info.num_trials)
